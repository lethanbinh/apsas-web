...
const SemesterPlanCardGrid: React.FC<SemesterPlanCardGridProps> = ({ semesters, loading, error }) => {
    if (loading) {
...
export default function SemesterPlans() {
  ...
  const filterSemestersByTab = (key: string): Semester[] => {
      if (key === '1') { 
          return semesters; 
      }
      return []; 
  ...
  const tabItems: TabsProps["items"] = [
    { 
        key: "1", 
        label: "Ongoing", 
        children: <SemesterPlanCardGrid 
            semesters={filterSemestersByTab('1')} 
            loading={loading} 
            error={error} 
        /> 
    },
    { 
        ...
        label: "Ended", 
        children: <SemesterPlanCardGrid 
            semesters={filterSemestersByTab('2')} 
            loading={loading} 
            error={error} 
        /> 
    },
    { 
        ...
        label: "Upcoming", 
        children: <SemesterPlanCardGrid 
            semesters={filterSemestersByTab('3')} 
            loading={loading} 
            error={error} 
        /> 
    },
    { 
        ...
        label: "Drafts", 
        children: <SemesterPlanCardGrid 
            semesters={filterSemestersByTab('4')} 
            loading={loading} 
            error={error} 
        /> 
    },
