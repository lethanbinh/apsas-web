...
export const handleDownloadAll = async (
  ...
  if (labs.length === 0) {
    message.warning("No labs to download");
    return;
  }
  ...
  try {
    ...
    for (const { lab, template, submissions } of labs) {
      if (!submissions || submissions.length === 0) continue;

      const labFolder = zip.folder(lab.name.replace(/[^a-zA-Z0-9]/g, "_"));
      if (!labFolder) continue;

      // Download requirement files if template exists
      if (template?.id) {
        try {
          const requirementFolder = labFolder.folder("Requirements");
          ...
          console.error(`Failed to generate requirement for lab ${lab.id}:`, err);
        }
      }

      // Download submissions
      const submissionsFolder = labFolder.folder("Submissions");
    ...
    saveAs(blob, "Labs_All.zip");
    message.destroy();
    message.success("All labs downloaded successfully");
  } catch (err: any) {
    console.error("Download all error:", err);
    message.destroy();
    message.error(err.message || "Failed to download all labs");
  }
};
